...
class MainActivity: AppCompatActivity() , ReceiverCallback,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        outputTextLyrics = findViewById(R.id.outputTextLyrics)
        albumArt = findViewById(R.id.albumartimage)
        setSupportActionBar(toolbar)
    ...
    suspend fun showLyrics() {
        ...
        work()
        Log.d("showLyrics", "Lyrics size: ${lyrics.size}")

        val localShowLyricsThread = Thread {
            try {
                for (i in 0 until lyrics.size) {
                    if (Thread.interrupted()) {
                        Log.d("showLyrics", "Thread interrupted. Stopping.")
                        return@Thread
                    }

                    Log.d("Inside loop", "I is at $i")
                    Thread.sleep((times_new[i] * 1000).toLong())
                    runOnUiThread {
                        updateUi(lyrics[i])
                    }
                    ...
                    if (i + 1 == lyrics.size) {
                        Log.d("showLyrics", "Finished showing lyrics.")
                        return@Thread
                    }
                }
                ...
                runOnUiThread {
                    ...
                    outputTextLyrics.text = Html.fromHtml(ot, Html.FROM_HTML_MODE_COMPACT)
                }
            }
        }

        showLyricsThread = localShowLyricsThread
        showLyricsThread!!.start()
    }


    suspend fun getLyrics(): JSONObject? = suspendCoroutine { continuation ->
    ...
    fun updateUi(text: String) {
        ...
        val outputTextlyrics = binding.outputTextLyrics

        runOnUiThread {

            outputTextlyrics.text = Html.fromHtml(formattedText, Html.FROM_HTML_MODE_COMPACT)
        }
    }
    ...
    override fun metadataChanged(song: Song) {
        ...
        val api_key_default = "21062742510293ae06df230e7cd334d26ae4cf17646b37514af666"
